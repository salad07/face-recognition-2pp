from resize import load_dataset
...
class DataSet(object):
   ...
   def extract_data(self,path):
        #根据指定路径读取出图片、标签和类别数
        imgs,labels,counter = load_dataset(path)
        ...
        X_train = X_train.reshape(X_train.shape[0], 3, self.img_size, self.img_size)/255.0
        X_test = X_test.reshape(X_test.shape[0], 3, self.img_size, self.img_size) / 255.0
