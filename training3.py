...
#建立一个基于CNN的人脸识别模型
class Model(object):
    FILE_PATH = r"C:\Users\Asus\Desktop\app\bilibili\model\me_baba_model.h5"  #模型进行存储和读取的地方
    IMAGE_SIZE = 64   #模型接受的人脸图片一定得是128*128的
    ...
    def train_model(self):
        ...
        self.model.fit(self.dataset.X_train,self.dataset.Y_train,epochs=50,batch_size=20)
    ...
    def predict(self,img):
        #add image = resize_image(image) +from
        img = resize_image(img)
        img = img.reshape((1, 3, self.IMAGE_SIZE, self.IMAGE_SIZE))
...
if __name__ == '__main__':
    dataset = DataSet(r'C:\Users\Asus\Desktop\app\bilibili\data')
    model = Model()
    # model.load(file_path = './model/liziqiang.face.model.h5')
